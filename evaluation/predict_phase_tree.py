"""Dialogue-continuation prediction via PHASE-Tree hypernetwork-generated LoRA.

This script uses a PHASE-Tree SFT-finetuned hypernetwork checkpoint to
generate per-character LoRA adapters and run character-adapted inference.
...
The prompt does **not** contain the profile text — character adaptation is
...
Usage::
    ...
    python evaluation/predict_phase_tree.py --gpu 0 --data .../m2_raw_profile/random_test.json ... &
    python evaluation/predict_phase_tree.py --gpu 1 --data .../m6_phase_tree/random_test.json ... &
    wait
...
def _load_hypermod_lightweight(
    ...
    for k, v in _HYPERMOD_DEFAULTS.items():
...
def main():
    parser = argparse.ArgumentParser(
        description="Predict with PHASE-Tree hypernetwork-generated LoRA",
    )
    ...
    n_gpus = torch.cuda.device_count()
    print(f"\n{'=' * 60}", flush=True)
    print(f"  Mode        : Hypernetwork LoRA (PHASE-Tree)", flush=True)
...
def multi_main():
    ...
    Usage::
        ...
        python evaluation/predict_phase_tree.py --multi \\
    ...
    """
    parser = argparse.ArgumentParser(
        description="Multi-task PHASE-Tree hypernetwork LoRA: models loaded once per GPU",
    )
    ...
    print(f"\n{'=' * 60}", flush=True)
    print(f"  Mode       : Multi-task PHASE-Tree Hypernetwork LoRA (models loaded ONCE)",
          flush=True)
    ...
    # ==================================================================
    # PHASE 1: Pre-compute encoded embeddings (hypermod + emb loaded ONCE)
    # ==================================================================
    # Lightweight loader (~3 min, no 14GB base model). After embedding
    # all unique profiles and running task_encoder, the emb model is
    # freed.  Hypermod (~1GB) stays in GPU for on-the-fly LoRA gen in
    # Phase 3, coexisting with vLLM.
    # ==================================================================
    print(f"\n{'─' * 60}", flush=True)
    print("  PHASE 1: Pre-compute embeddings (hypermod + emb loaded ONCE)",
          flush=True)
    print(f"{'─' * 60}", flush=True)
    ...
    (
        ckpt_args, hypermod, peft_cfg, _model_dir,
        emb_model, emb_tokenizer, pooling_fn, n_layers,
    ) = _load_hypermod_lightweight(
        args.checkpoint, args.emb_model_override,
        args.model_override, args.device,
    )

    identity_fn = lambda x: x  # noqa: E731
    layer_indices = torch.tensor(
        range(n_layers), dtype=torch.long, device=args.device,
    )
    ...
    non_empty = [pt for pt in all_profiles if pt]
    non_empty_pos = {pt: i for i, pt in enumerate(non_empty)}
    hidden_size = emb_model.config.hidden_size
    ...
    with torch.no_grad():
        ...
        if non_empty:
            all_embs = embed_texts(
                non_empty, emb_model, emb_tokenizer,
                identity_fn, pooling_fn, args.device,
                batch_size=args.emb_batch_size,
            )
            if all_embs.device != torch.device(args.device):
                all_embs = all_embs.to(args.device)
        ...
        for pt in all_profiles:
            if pt and all_embs is not None:
                ...
                task_emb = torch.zeros(1, hidden_size, device=args.device)
            enc_out = hypermod.task_encoder(task_emb)
            encoded_lookup[pt] = enc_out["encoded_task_emb"].detach()

    emb_elapsed = time.perf_counter() - t_emb

    del all_embs, emb_model, emb_tokenizer, pooling_fn, identity_fn
    torch.cuda.empty_cache()
    gc.collect()

    print(f"  Embedded + encoded {len(all_profiles)} profiles in "
          f"{emb_elapsed:.1f}s", flush=True)
    print(f"  Freed emb_model; keeping hypermod (~1GB) for per-task LoRA gen",
          flush=True)

    # ==================================================================
    # PHASE 2: Load vLLM ONCE (hypermod ~1GB coexists in GPU)
    # ==================================================================
    print(f"\n{'─' * 60}", flush=True)
    ...
    # ==================================================================
    # PHASE 3: Per-task LoRA generation + inference
    # ==================================================================
    # Hypermod (~1GB) + vLLM coexist in GPU.  For each task, LoRAs are
    # generated from pre-computed encoded embeddings, used for inference,
    # then deleted.  Peak disk = max_task_profiles * ~6MB (bounded per
    # task, not cumulative).  Models never reloaded.
    # ==================================================================
    lora_id_counter = 1
    all_gen_seconds: list[float] = []
    ...
    for task_idx, td in enumerate(task_data):
        ...
        if not remaining:
            print(f"\n  [{task_idx}] {label}/{split}: already complete",
                  flush=True)
            continue

        task_profiles = list(set(
            s.get("profile_text", "") for s in remaining
        ))
        n_task_profiles = len(task_profiles)
        print(f"\n  [{task_idx}] {label}/{split}: {len(remaining)} samples, "
              f"{n_task_profiles} unique profiles ...", flush=True)

        _afs_temp = os.path.join(tc["output_dir"], "_temp_loras")
        lora_base_dir = _get_ramdisk_lora_dir(_afs_temp)
        os.makedirs(lora_base_dir, exist_ok=True)
        ...
        # Generate LoRAs for this task's unique profiles
        profile_to_lora: dict[str, str] = {}
        temp_dirs: list[str] = []
        task_gen_seconds: list[float] = []

        with torch.no_grad():
            for p_idx, pt in enumerate(task_profiles):
                profile_hash_val = hashlib.sha256(
                    pt.encode()).hexdigest()[:16]

                if save_dir:
                    perm_dir = os.path.join(
                        save_dir, f"lora_{profile_hash_val}")
                    if os.path.isdir(perm_dir) and os.path.isfile(
                        os.path.join(perm_dir,
                                     "adapter_model.safetensors")
                    ):
                        profile_to_lora[pt] = perm_dir
                        continue

                lora_dir = os.path.join(
                    lora_base_dir, f"lora_{profile_hash_val}")
                ...
                lora_sd = hypermod.gen_lora(
                    layer_indices, encoded_lookup[pt])
                gen_t = time.perf_counter() - t0
                task_gen_seconds.append(gen_t)
                all_gen_seconds.append(gen_t)
                ...
                save_lora(lora_sd, hypermod.peft_config, lora_dir)
                profile_to_lora[pt] = lora_dir
                temp_dirs.append(lora_dir)

                if save_dir:
                    perm_dir = os.path.join(
                        save_dir, f"lora_{profile_hash_val}")
                    if not os.path.exists(perm_dir):
                        shutil.copytree(lora_dir, perm_dir)
                    profile_to_lora[pt] = perm_dir

                if (p_idx + 1) % 200 == 0 or \
                        (p_idx + 1) == n_task_profiles:
                    print(f"    LoRA {p_idx + 1}/{n_task_profiles} "
        ...
        if save_dir:
            ...
            print(f"  Per-character LoRAs: {save_dir} ({n_saved} total)",
                  flush=True)

        # Build LoRA request registry for vLLM
        task_lora_dirs = set(profile_to_lora.values())
        lora_requests: dict[str, LoRARequest] = {}
        for lora_d in task_lora_dirs:
        ...
        for g_idx, (lora_d, group) in enumerate(lora_groups.items()):
            indices, prompts = zip(*group)
        ...
        speed = len(remaining) / infer_elapsed if infer_elapsed > 0 else 0
        print(f"    Done: {len(remaining)} samples in {infer_elapsed:.1f}s "
              f"({speed:.1f} samples/s)", flush=True)

        total_gen = sum(task_gen_seconds) if task_gen_seconds else 0
        gen_stats = {
            "num_unique_profiles": n_task_profiles,
            "num_generated": len(task_gen_seconds),
            "num_cached": n_task_profiles - len(task_gen_seconds),
            "gen_lora_seconds": round(total_gen, 2),
            "lora_rank": lora_rank,
            "model_dir": inference_model_dir,
        }
        meta = {
            "mode": "phase_tree_lora_multi",
            "model": inference_model_dir,
            "checkpoint": args.checkpoint,
            "temperature": args.temperature,
            "max_tokens": args.max_tokens,
            ...
            "inference": {
                ...
                "mean_ms_per_sample": round(
                    infer_elapsed / len(remaining) * 1000, 1,
                ) if remaining else 0,
        ...
        # Cleanup temp LoRAs for this task (persistent save_dir untouched)
        for tmp in [lora_base_dir, _afs_temp]:
            if os.path.exists(tmp):
                shutil.rmtree(tmp, ignore_errors=True)
