#!/usr/bin/env bash
# ===========================================================================
# PHASE-Tree Prompt Predictions — Qwen3-32B backbone (multi-dataset)
# ===========================================================================
# Reuses evaluation/predict_prompt.py (--multi) to run the same explicit
# textual-provision methods as results/*/prompt/main, but with Qwen3-32B.
#
# Outputs go to a separate tree so the original Qwen2.5-7B results are kept:
#   results/<DATASET>/prompt/qwen3_32b/<METHOD>/<SPLIT>/predictions.jsonl
#
# Default: all 8 LongEvoRoleBench datasets, methods matching prompt/main,
# GPUs 0-7 (one vLLM worker per GPU, tensor_parallel=1).
#
# Usage:
#   bash evaluation/run_prompt_predict_qwen3_32b.sh
#   DATASETS="CharacterEval RAIDEN" bash evaluation/run_prompt_predict_qwen3_32b.sh
#   NUM_SAMPLES=8 bash evaluation/run_prompt_predict_qwen3_32b.sh   # smoke test
#
# Environment knobs (all optional):
#   DATASETS              space-separated (default: all 8)
#   MODEL                 default /mnt/lstore/model/Qwen3-32B
#   RESULTS_TAG           default qwen3_32b  → results/<DS>/prompt/<TAG>/
...
#   BATCH_SIZE            default 16 (passed through; vLLM batches internally)
...
RESULTS_TAG="${RESULTS_TAG:-qwen3_32b}"
