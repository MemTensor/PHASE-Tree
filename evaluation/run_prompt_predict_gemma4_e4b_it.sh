#!/usr/bin/env bash
# ===========================================================================
# PHASE-Tree Prompt Predictions — Gemma-4-E4B-it backbone (multi-dataset)
# ===========================================================================
# Reuses evaluation/predict_prompt.py (--multi) to run the same explicit
# textual-provision methods as results/*/prompt/main, but with Gemma-4-E4B-it.
#
# Outputs go to a separate tree so the original Qwen2.5-7B results are kept:
#   results/<DATASET>/prompt/gemma_4_e4b_it/<METHOD>/<SPLIT>/predictions.jsonl
#
# Default: all 8 LongEvoRoleBench datasets, methods matching prompt/main,
# GPUs 0-7 (one vLLM worker per GPU, tensor_parallel=1).
#
# Usage:
#   bash evaluation/run_prompt_predict_gemma4_e4b_it.sh
#   DATASETS="CharacterEval RAIDEN" bash evaluation/run_prompt_predict_gemma4_e4b_it.sh
#   NUM_SAMPLES=8 bash evaluation/run_prompt_predict_gemma4_e4b_it.sh   # smoke test
#
# Environment knobs (all optional):
#   DATASETS              space-separated (default: all 8)
#   MODEL                 default /mnt/lstore/model/gemma-4-E4B-it
#   RESULTS_TAG           default gemma_4_e4b_it  → results/<DS>/prompt/<TAG>/
...
#   BATCH_SIZE            default 32 (passed through; vLLM batches internally)
