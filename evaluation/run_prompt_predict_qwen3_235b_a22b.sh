#!/usr/bin/env bash
# ===========================================================================
# PHASE-Tree Prompt Predictions — Qwen3-235B-A22B backbone (multi-dataset)
# ===========================================================================
# Reuses evaluation/predict_prompt.py (--multi) to run the same explicit
# textual-provision methods as results/*/prompt/main, but with Qwen3-235B-A22B.
#
# Outputs go to a separate tree so the original Qwen2.5-7B results are kept:
#   results/<DATASET>/prompt/qwen3_235b_a22b/<METHOD>/<SPLIT>/predictions.jsonl
#
# Default: all 8 LongEvoRoleBench datasets, methods matching prompt/main.
# 235B-A22B MoE requires tensor_parallel=8 across all 8 GPUs (1 worker).
#
# Usage:
#   bash evaluation/run_prompt_predict_qwen3_235b_a22b.sh
#   DATASETS="CharacterEval RAIDEN" bash evaluation/run_prompt_predict_qwen3_235b_a22b.sh
#   NUM_SAMPLES=8 bash evaluation/run_prompt_predict_qwen3_235b_a22b.sh   # smoke test
#
# Environment knobs (all optional):
#   DATASETS              space-separated (default: all 8)
#   MODEL                 default /mnt/lstore/model/Qwen3-235B-A22B
#   RESULTS_TAG           default qwen3_235b_a22b  → results/<DS>/prompt/<TAG>/
#   GPU_IDS               default "0 1 2 3 4 5 6 7"
#   NUM_GPU_WORKERS       default 1 (235B needs all 8 GPUs via TP)
...
for gpu_slot in $(seq 0 $((NUM_GPU_WORKERS - 1))); do
    ...
    if [ "$TENSOR_PARALLEL" -gt 1 ]; then
        WORKER_CUDA_DEVICES="$CUDA_DEV_LIST"
        WORKER_LABEL="tp${TENSOR_PARALLEL}"
    else
        WORKER_CUDA_DEVICES="$PHYSICAL_GPU"
        WORKER_LABEL="gpu${PHYSICAL_GPU}"
    fi
    TASK_FILE="${GLOBAL_TASKS_DIR}/tasks_${WORKER_LABEL}.json"
    ENTRIES="["
    FIRST=true
    n_gpu_tasks=0
    ...
    LOG_FILE="${GLOBAL_LOG_DIR}/predict_${WORKER_LABEL}.log"
    echo "  [${WORKER_LABEL}] ${n_gpu_tasks} tasks -> ${TASK_FILE}"
    echo "             CUDA_VISIBLE_DEVICES=${WORKER_CUDA_DEVICES}"
    echo "             log: ${LOG_FILE}"

    CUDA_VISIBLE_DEVICES="$WORKER_CUDA_DEVICES" $PYTHON evaluation/predict_prompt.py \
        --multi \
        --tasks "$TASK_FILE" \
        --model "$MODEL" \
        --backend "$BACKEND" \
    ...
    PREDICT_PIDS+=($!)
    PREDICT_DESCS+=("${WORKER_LABEL} (${n_gpu_tasks} tasks)")
done
