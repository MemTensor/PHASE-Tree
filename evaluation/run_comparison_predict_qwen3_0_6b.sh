#!/usr/bin/env bash
# ===========================================================================
# PHASE-Tree Comparison Baselines — Qwen3-0.6B (multi-dataset, 8-GPU)
# ===========================================================================
# Runs rag / pag / cfg predictions for all 8 LongEvoRoleBench datasets with
# Qwen3-0.6B, mirroring results/*/comparison/main/{rag,pag,cfg} layout but
# writing to a separate tag so Qwen2.5-7B main results are preserved:
#   results/<DATASET>/comparison/qwen3_0_6b/<METHOD>/<SPLIT>/predictions.jsonl
#
# Usage:
#   bash evaluation/run_comparison_predict_qwen3_0_6b.sh
#   METHODS="rag pag" DATASETS="CharacterEval" bash evaluation/run_comparison_predict_qwen3_0_6b.sh
#   NUM_SAMPLES=8 bash evaluation/run_comparison_predict_qwen3_0_6b.sh   # smoke test
#
...
#   RESULTS_TAG           default qwen3_0_6b
#   GPU_IDS               default "0 1 2 3 4 5 6 7"
#   TOP_K                 default 3 (matches existing comparison/main)
#   CFG_BATCH_SIZE        default 32 (0.6B model; CFG is memory-light)
...
CFG_BATCH_SIZE="${CFG_BATCH_SIZE:-32}"
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║  Comparison Predictions — ${RESULTS_TAG} (Qwen3-0.6B)"
echo "╠══════════════════════════════════════════════════════════════╣"
echo "║  Model      : ${MODEL}"
