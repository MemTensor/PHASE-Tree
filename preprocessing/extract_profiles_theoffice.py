"""Extract initial character profiles for The Office's 6 main characters using LLM.

The Office provides only dialogue transcripts — no explicit character profiles.
This script:
...
4. Saves profiles using the same unified field schema as Friends/ChatHaruhi.

Input
    ``LongEvoRoleBench/raw_data/TheOffice/the_office_season_01.json``

Output
    ``LongEvoRoleBench/processed/TheOffice/intermediate/raw_profiles.json``
...
Usage::
    ...
    python extract_profiles_theoffice.py
    python extract_profiles_theoffice.py --seasons 1 2 --n_sample 40
"""
...
excerpts from Season 1 of the TV show "The Office" (US version) featuring \
a specific character. Your task is to analyse their speech patterns, \
behaviour, and relationships **as they appear in these early episodes only** \
to produce a comprehensive character profile in JSON.
...
- **age**: Age or age range if inferable (e.g. "mid-30s"), otherwise null.
- **occupation**: Job title, role, or social position.
...
"role is Name" (e.g. "boss is Michael, desk neighbor is Jim").
...
def generate_profile(
    ...
    user_prompt = (
        ...
        f"Below are {len(sampled)} scene excerpts from Season 1 of The Office "
        f"featuring this character:\n\n{dialogues_text}\n\n"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Extract initial character profiles for The Office (6 main characters)",
    )
