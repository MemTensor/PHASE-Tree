"""Preprocess Star Trek TNG multi-party dialogues into PHASE-Tree samples with temporal split.

**Step 1 — Dialogue conversion.**  For each scene across all 7 seasons,
...
* ``train``        — Seasons 1–5 (random 80% of qualifying samples)
* ``random_test``  — Seasons 1–5 (random 20% holdout, same epoch)
* ``ood_test``     — Seasons 6–7 (later time period — for testing
...
Usage::
    ...
    python preprocess_dialogues_tng.py
    python preprocess_dialogues_tng.py --train_seasons 1 2 3 4 5 \\
                                       --test_seasons 6 7
"""
...
def _stable_id(scene_id: str, utt_idx: int) -> str:
    ...
    raw = f"StarTrek_TNG_{scene_id}_{utt_idx}"
    short_hash = hashlib.md5(raw.encode()).hexdigest()[:12]
    return f"TNG_{short_hash}"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Preprocess Star Trek TNG dialogues into PHASE-Tree samples "
                    "with temporal train/test split",
    )
    ap.add_argument("--train_seasons", type=int, nargs="+",
                    default=list(range(1, 6)),
                    help="Seasons for train + random_test (default: 1-5)")
    ap.add_argument("--test_seasons", type=int, nargs="+",
                    default=[6, 7],
                    help="Seasons for ood_test (default: 6-7)")
