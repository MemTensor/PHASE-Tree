"""Extract initial character profiles for Friends' 6 main characters using LLM.

Friends provides only dialogue transcripts — no explicit character profiles.
This script:
...
Usage::
    ...
    python extract_profiles_friends.py
    python extract_profiles_friends.py --seasons 1 2 --n_sample 40
"""
...
excerpts from Season 1 of the TV show "Friends" featuring a specific \
character. Your task is to analyse their speech patterns, behaviour, and \
relationships **as they appear in these early episodes only** to produce \
a comprehensive character profile in JSON.
...
- **age**: Age or age range if inferable (e.g. "mid-20s"), otherwise null.
...
"role is Name" (e.g. "best friend is Joey, sister is Monica").
...
def collect_scenes(season_nums: list[int]) -> list[dict]:
    """Return flat list of scenes across requested seasons."""
    scenes = []
...
def format_scene(scene: dict, max_utts: int = 30) -> str:
    """Format a scene into readable dialogue text."""
    lines = []
...
def generate_profile(
    ...
    user_prompt = (
        ...
        f"Below are {len(sampled)} scene excerpts from Season 1 of Friends "
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Extract initial character profiles for Friends (6 main characters)",
    )
