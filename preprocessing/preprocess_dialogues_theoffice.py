"""Preprocess The Office multi-party dialogues into PHASE-Tree samples with temporal split.

**Step 1 — Dialogue conversion.**  For each scene across all 9 seasons,
...
* ``train``        — Seasons 1–6 (random 80% of qualifying samples)
* ``random_test``  — Seasons 1–6 (random 20% holdout, same epoch)
* ``ood_test``     — Seasons 7–9 (later time period — for testing
                     dynamic attribute tree evolution; includes Michael's
                     departure after S7 and Andy taking over as manager)

Input
    ``LongEvoRoleBench/raw_data/TheOffice/the_office_season_*.json``

Output
    ``LongEvoRoleBench/processed/TheOffice/intermediate/all_dialogues.json``
    ``LongEvoRoleBench/processed/TheOffice/intermediate/{train,random_test,ood_test}.json``

Usage::

    python preprocess_dialogues_theoffice.py
    python preprocess_dialogues_theoffice.py --train_seasons 1 2 3 4 5 6 \\
                                             --test_seasons 7 8 9
"""
...
def load_season(season_num: int) -> dict:
    ...
    path = RAW_DATA_DIR / "TheOffice" / f"the_office_season_{season_num:02d}.json"
...
def _stable_id(scene_id: str, utt_idx: int) -> str:
    """Generate a deterministic question_id from scene and utterance position."""
    raw = f"TheOffice_{scene_id}_{utt_idx}"
    short_hash = hashlib.md5(raw.encode()).hexdigest()[:12]
    return f"TheOffice_{short_hash}"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Preprocess The Office dialogues into PHASE-Tree samples "
                    "with temporal train/test split",
    )
    ap.add_argument("--train_seasons", type=int, nargs="+",
                    default=list(range(1, 7)),
                    help="Seasons for train + random_test (default: 1-6)")
    ap.add_argument("--test_seasons", type=int, nargs="+",
                    default=[7, 8, 9],
                    help="Seasons for ood_test (default: 7-9)")
    ...
    out_dir = PROCESSED_DIR / "TheOffice" / "intermediate"
